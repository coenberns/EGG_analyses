...
def read_egg_v3_bursts(file,
    ...
    t_cycle = (groupdat['elapsed_s'].max())/((groupdat['packet_miss_idx'].max()+1)/(n_burst+sleep_ping))
    tarray=[]
    ...
    return groupdat, groupdat_mean, times

# %% Getting data for raw values 
groupdats = {}

for i in range(1, 4):
    result = read_egg_v3_bursts(file, func=i, mean=False)
    groupdat = result[0]
...
for i in range(1, 4):
    result = read_egg_v3_bursts(file, func=i, mean=True)
    groupdat_mean = result[1]
    groupdats_mean[f'function{i}'] = groupdat_mean

# %%
# groupdat, _, times = read_egg_v3_bursts(file,
#                                         header=0,
#                                         rate = 62.5,
#                                         scale=600,
#                                         error=0,
...
# _, groupdat_mean, _ = read_egg_v3_bursts(file,
#                                         header=0,
#                                         rate = 62.5,
#                                         scale=600,
#                                         error=0,
...
# # num_rows_na_mean = dat_mean.isna().any(axis=1).sum()
# # perc_na_mean = (num_rows_na_mean/ len(dat_mean)) * 100
# # print(f"The percentage of rows with an na value is: {perc_na_mean}%")
...
# # # signalplot(dat=dat_mean,skip_chan=[0,1,2,4], freq=[0.025,0.5],
# # #             rate=times['effective_rate'], spacer=20)
# # # signalplot(dat=dat_mean_noint,skip_chan=[0,1,2,4], freq=[0.025,0.5],
# # #             rate=times['effective_rate'], spacer=20)

# # %%

# # egg_signalfreq(dat_mean, rate=times['effective_rate'])
# # egg_signalfreq(dat_mean_noint, rate=times['effective_rate'])
...
# # egg_signalfreq(dat, rate=times['effective_rate']*5)
...
# peakf = extract_peak_frequency(dat_mean['Channel 2'], times['effective_rate'])
