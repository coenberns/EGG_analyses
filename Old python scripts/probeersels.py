...
file = r'C:/Users/Coen/OneDrive - Mass General Brigham/Documents/Thesis/Measurements/Pig measurements/08152023_First_short_pigmeasurement/08152023_PowerCycle.txt'
...
# %%
# MASTERFUNCTION for low power


def read_egg_v3_lowerP(file, header=0, scale=600, error=0, date=None, n_burst=5, sleep_time=2, t_deviation=0.2, n_missing=1):
    """
    ...
    VT_data['timestamps'] = (VT_data['packet_re_idx'] /
                             (n_burst+1))*avg_time_between

    return VT_data, grouped_fulldat, volt_fulldat
...
# Continuation of last part of the reading function
final_fulldat = grouped_fulldat.dropna(subset=['group'])
final_fulldat = final_fulldat.reset_index(drop=True)
second_counter = final_fulldat.packet_re_idx.astype(int)
second_counter = second_counter.reset_index(drop=True)
re_counter = [0]
for j, ele in enumerate(second_counter[1:]):
    step = second_counter[j+1]-second_counter[j]
    if step > 0:
        re_counter.append(step+re_counter[j])
    else:
        re_counter.append(
            65536-second_counter[j]+second_counter[j+1]+re_counter[j])
        print('flip', step, 65536-second_counter[j]+second_counter[j+1])


tarray = np.array(re_counter)*1/effective_rate
new_abscounterseries = pd.Series(re_counter, name='counter')
tseries = pd.Series(tarray, name='timestamps')


final_fulldat = pd.concat(
    (final_fulldat, new_abscounterseries, tseries), axis=1)


def detect_missing_packets(packet_series):
    # Difference between consecutive packet numbers
    diff = packet_series.diff().shift(-1)

    missing_indices = []

    # Iterate through the diffs to detect missing packets and handle rollovers
    for idx, val in enumerate(diff):
        if val < 0:  # Rollover detected as 1-65536 is negative
            expected_next_packet = packet_series.iloc[idx] + 1
            actual_next_packet = packet_series.iloc[idx + 1]
            missing_count = (actual_next_packet + 65536) - expected_next_packet
            if missing_count > 0:
                missing_indices.append(packet_series.iloc[idx])

        elif val > 1:  # Missing packets without rollover
            missing_indices.append(packet_series.iloc[idx])
