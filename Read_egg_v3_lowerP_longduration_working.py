...
def read_egg_v3_lowerP(file,
    ...
    #NON-MEAN PRE-PROCESSING
        ...
        grouped_fulldat = grouped_fulldat[grouped_fulldat['misc'] == " 16 "]
        groupdat = grouped_fulldat.copy()
        ...
        delta_t_finish = abs(volt_fulldat['elapsed_s'].max()-volt_fulldat['timestamps'].max())/(volt_fulldat['packet_start_idx'].max()-1)
        ...
        volt_fulldat['corrected_timestamps'] = volt_fulldat['timestamps'] + delta_t_series
        #Second method
        ...
        tseries2 = pd.Series(tarray_2, name='timestamps_tcycle')
        ...
        tseries3 = pd.Series(tarray_3, name='timestamps_both')

        volt_fulldat = pd.concat([volt_fulldat,tseries2,tseries3],axis=1)
        delta_t_combi = abs(volt_fulldat['elapsed_s'].max()-volt_fulldat['timestamps_both'].max())/(volt_fulldat['packet_start_idx'].max()-1)
        delta_t_combi_series = pd.Series([delta_t_combi]*len(volt_fulldat))
        delta_t_combi_series = delta_t_combi_series.cumsum()
        delta_t_combi_series -= delta_t_combi
        volt_fulldat['corrected_t_both'] = volt_fulldat['timestamps_both']+delta_t_combi_series
        datcols = ['corrected_realtime','elapsed_s','timestamps', 'corrected_timestamps', 'corrected_t_both',
                   'timestamps_tcycle','timestamps_both', 'packet_start_idx'] + [f'Channel {i}' for i in range(8)]
        VT_data = volt_fulldat[datcols]
...
Read_egg_v3_lowerP_longduration_working.py
Read_egg_v3_lowerP_longduration_working.py wordt weergegeven.